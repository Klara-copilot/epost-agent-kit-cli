...
The CLI implements a clean 4-layer architecture with ~7,130 lines of code:
...
└───────────────┬─────────────────────────┘
                │
┌───────────────▼─────────────────────────┐
│        Commands Layer (2,358 LOC)       │  src/commands/
│  Thin orchestrators, user interaction   │  11 commands
└───────────────┬─────────────────────────┘
                │
┌───────────────▼─────────────────────────┐
│        Domains Layer (3,619 LOC)        │  src/domains/
│  Business logic, algorithms, workflows  │  10 domains
└───────────────┬─────────────────────────┘
                │
┌───────────────▼─────────────────────────┐
...
**Size:** 2,358 LOC across 11 files
...
**Commands:**
1. `init.ts` (771 LOC) - Initialize kit (dual mode: package/legacy)
2. `package.ts` (235 LOC) - Manage packages (list/add/remove)
3. `dev.ts` (228 LOC) - File watcher for kit designers
4. `onboard.ts` (220 LOC) - Guided setup wizard
5. `uninstall.ts` (214 LOC) - Remove kit with ownership awareness
6. `new.ts` (162 LOC) - Create project from template
7. `profile.ts` (138 LOC) - Browse and show profiles
8. `versions.ts` (119 LOC) - List available kit versions
9. `doctor.ts` (110 LOC) - Health checks with auto-fix
10. `update.ts` (92 LOC) - Self-update CLI from npm
11. `workspace.ts` (69 LOC) - Generate workspace CLAUDE.md
...
**Size:** 3,619 LOC across 10 domains
**Purpose:** Core business logic and algorithms

**Domains:**

**a) Packages (`packages/`)** - 843 LOC
- `package-resolver.ts` (512 LOC) - Manifest loading, dependency resolution
- `profile-loader.ts` (328 LOC) - Auto-detection, profile enumeration

**b) Installation (`installation/`)** - 704 LOC
...
**c) GitHub (`github/`)** - 726 LOC
- `github-client.ts` - Fetch releases from GitHub API
- `release-downloader.ts` - Download tarball assets
- `release-cache.ts` - Cache management for releases
- `release-validator.ts` - Validate release integrity
- `access-checker.ts` - Verify gh CLI authentication
...
- `branding.ts` - Logo, tagline, version display
- Display helpers for command output

**f) Config (`config/`)** - 268 LOC
- `config-loader.ts` - Configuration file discovery
- `settings-merger.ts` - Combine settings from multiple packages
- Strategy: base (first), merge (deep merge), skip (ignore)

**g) Health Checks (`health-checks/`)** - 181 LOC
...
- Commands: Target < 200 LOC (exceptions: `init.ts` at 771 LOC due to dual mode)
...
- [Codebase Integrations & Operations](./codebase-integrations.md) - External integrations, config, testing, build/deploy, security
